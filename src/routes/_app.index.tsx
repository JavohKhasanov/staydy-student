...
function Home() {
  const today = todaysLessons();
  const pending = upcomingHomework();
  ...
  return (
    <div className="space-y-6 px-5 pb-8 pt-4">
      ...
      <section className="animate-card-rise">
        ...
        <div className="mt-4 flex items-center justify-between rounded-2xl border border-hairline bg-elevated p-4">
          <div>
            ...
            <p className="font-display font-bold">+{student.xpToNext - student.xp} XP kerak</p>
          </div>
          <Link
            to="/reyting"
      ...
      {nextDeadline && (
        <Link
          to="/darslar/$lessonId"
          params={{ lessonId: nextDeadline.lessonId }}
        ...
        >
          <div className="flex items-center gap-3">
            ...
            <div className="flex-1">
              ...
              <p className="text-xs text-alert/80 tabular">
                {timeRemaining(nextDeadline.deadline).label}
              </p>
            </div>
      ...
      <section className="animate-card-rise space-y-3" style={{ animationDelay: "180ms" }}>
        <div className="flex items-end justify-between">
          ...
          <span className="text-xs text-muted-foreground">{today.length} ta dars</span>
        </div>

        {today.length === 0 ? (
          <div className="rounded-2xl border border-dashed border-hairline bg-surface/50 p-6 text-center">
            <p className="text-sm text-muted-foreground">Hali dars yo'q. Dam oling 🙂</p>
          </div>
        ) : (
          today.map((l) => {
            const group = getGroup(l.groupId);
            const statusMap = {
              now: { label: "Hozir", cls: "bg-success/15 text-success border-success/30" },
              upcoming: { label: "Boshlanmagan", cls: "bg-white/5 text-muted-foreground border-hairline" },
              done: { label: "Bo'ldi", cls: "bg-primary/15 text-primary border-primary/30" },
            }[l.status];
            return (
              <Link
                key={l.id}
              ...
              >
                <div className="mb-3 flex items-start justify-between gap-2">
                  <span className={`rounded-full border px-2.5 py-0.5 text-[10px] font-bold uppercase tracking-wider ${statusMap.cls}`}>
                    {statusMap.label}
                  </span>
                  <span className="flex items-center gap-1 text-xs text-muted-foreground tabular">
                    <Clock className="size-3" /> {l.timeLabel}
                  </span>
                </div>
                <h3 className="font-display text-base font-bold">{l.topic}</h3>
                <p className="mt-1 text-xs text-muted-foreground">
                  {group?.name} · {group?.teacher.name} · {l.room}
                </p>
              </Link>
            );
          })
      ...
      <section className="animate-card-rise grid grid-cols-3 gap-3" style={{ animationDelay: "240ms" }}>
        {[
          { label: "Davomat", value: "92%", color: "text-foreground" },
          { label: "Vazifalar", value: "24/30", color: "text-success" },
          { label: "O'rtacha", value: "4.8", color: "text-reward" },
        ].map((s) => (
